...
class Train:
    ...
    def calculate_accuracy(self, current_reward):
        """
        Calculate accuracy based on the range of rewards observed so far.
        Normalizes rewards to a scale of 0 to 100%.
        """
        if not self.rewards:
        ...
        if max_reward == min_reward:
            return 100 if current_reward >= max_reward else 0

        # Normalize current_reward to a 0-100% scale
        accuracy = ((current_reward - min_reward) / (max_reward - min_reward)) * 100
        return max(0, min(accuracy, 100))  # Ensure accuracy is within 0-100%
    ...
    def train(self):
        for generation in range(self.generations):
            ...
            # Calculate accuracy based on normalized rewards
